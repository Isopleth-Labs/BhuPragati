...
const FLOOD_METRICS = [
  ...
  { label: "Affected Population", value: "2.8M", change: "+7.3%", trend: "alert", progress: 68 },
  { label: "Rainfall (24h)", value: "118.6 mm", change: "+12.4%", trend: "alert" },
];
    ...
    <aside className={`hero__panel hero__panel--${side}`} aria-label={title}>
      <div className="stat-panel">
        <div className="stat-panel__header">
          <span className={`stat-panel__symbol stat-panel__symbol--${type}`} aria-hidden="true" />
          <span>{title}</span>
        </div>
        ...
        {metrics.map((metric, index) => (
          <div className="stat-panel__row" key={metric.label}>
            ...
            <p className="stat-panel__value">
              ...
              <span className={`stat-panel__change stat-panel__change--${metric.trend}`}>
                {metric.change}
