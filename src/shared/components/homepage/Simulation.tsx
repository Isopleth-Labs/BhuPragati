...
function buildInfrastructureData() {
	const regions: Array<{ id: number; x: number; y: number; minTier: number; size: number; type: string }> = []
	const corridors: Array<{ id: number; r1: (typeof regions)[0]; r2: (typeof regions)[0]; waypoints: Array<{ x: number; y: number }>; minTier: number; type: string; length: number }> = []
	let id = 0

	const addRegion = (x: number, y: number, minTier: number, size: number, type = "hub") => {
		const region = { id: id++, x, y, minTier, size, type }
		regions.push(region)
		return region
	}

	const addCorridor = (r1: (typeof regions)[0], r2: (typeof regions)[0], minTier: number, type = "highway") => {
		if (!r1 || !r2) return
...
function NetworkCanvas({
	...
	useEffect(() => {
		...
		const render = (time: number) => {
			if (!isVisible) { lastTime = time; animationFrameId = requestAnimationFrame(render); return }
			...
			INFRA_DATA.regions.forEach((reg) => {
				if (reg.minTier <= stepIndex) {
					const cx = mapX(reg.x), cy = mapY(reg.y)
					let r = 20 + reg.size * 25
					if (reg.minTier === stepIndex && stepIndex > 0) r *= 0.2 + 0.8 * hp
					else r *= 1.0 + 0.2 * hp
					const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, r)
					const zoneColor = reg.minTier === 0 && stepIndex === 0 ? theme.colorRGB : reg.minTier === 1 && stepIndex === 1 ? theme.colorRGB : stepIndex === 2 ? theme.colorRGB : "255,255,255"
			...
			INFRA_DATA.corridors.forEach((c) => {
				...
				let lineAlpha = 0.25, lineWidth = c.type === "highway" ? 3 : 2, isDashed = false, drawPercent = 1.0
				if (isBuilding) {
					if (hp < 0.05) { isDashed = true; lineAlpha = 0.15; lineWidth = 1.5 }
					else { isDashed = true; lineAlpha = 0.4 * hp; lineWidth = 2 + 1 * hp; drawPercent = hp }
				} else { lineAlpha = 0.25 + 0.2 * hp; lineWidth = c.type === "highway" ? 3 + hp : 2 + hp }

				const drawPath = (percent: number, dashArray: number[], colorStr: string, width: number) => {
					ctx.beginPath(); ctx.lineWidth = width; ctx.strokeStyle = colorStr; ctx.setLineDash(dashArray)
					let totalLen = 0
					const segments: Array<{ sx: number; sy: number; ex: number; ey: number; l: number }> = []
					for (let i = 0; i < path.length - 1; i++) {
						const sx = mapX(path[i].x), sy = mapY(path[i].y), ex = mapX(path[i + 1].x), ey = mapY(path[i + 1].y)
						const l = Math.hypot(ex - sx, ey - sy)
						segments.push({ sx, sy, ex, ey, l }); totalLen += l
					}
					const targetLen = totalLen * percent; let currentLen = 0
					if (segments.length > 0) {
						ctx.moveTo(segments[0].sx, segments[0].sy)
						for (const seg of segments) {
							if (currentLen + seg.l <= targetLen) { ctx.lineTo(seg.ex, seg.ey); currentLen += seg.l }
							else { const p = (targetLen - currentLen) / seg.l; ctx.lineTo(seg.sx + (seg.ex - seg.sx) * p, seg.sy + (seg.ey - seg.sy) * p); break }
						}
					}
				...
				if (!isBuilding && hp > 0.1) drawPath(1.0, [], `rgba(${col}, ${lineAlpha * 0.8 * hp})`, lineWidth * 0.4)

				if (!isBuilding && hp > 0) {
					const pulseProgress = (time * 0.0005 + c.id * 0.3) % 1
					if (pulseProgress < 1.0) {
						let currentLen = 0, totalLen = 0
						const segments: Array<{ sx: number; sy: number; ex: number; ey: number; l: number }> = []
						for (let i = 0; i < path.length - 1; i++) {
							const sx = mapX(path[i].x), sy = mapY(path[i].y), ex = mapX(path[i + 1].x), ey = mapY(path[i + 1].y)
							const l = Math.hypot(ex - sx, ey - sy)
						...
						for (const seg of segments) {
							if (currentLen + seg.l <= targetLen) { currentLen += seg.l }
							else { const p = (targetLen - currentLen) / seg.l; px = seg.sx + (seg.ex - seg.sx) * p; py = seg.sy + (seg.ey - seg.sy) * p; break }
						}
						if (px && py) {
							ctx.beginPath()
							...
							ctx.fillStyle = pulseGlow; ctx.arc(px, py, 20, 0, Math.PI * 2); ctx.fill()
						}
					}
				}
			})
			...
			INFRA_DATA.regions.forEach((reg) => {
				...
				const x = mapX(reg.x), y = mapY(reg.y)
				let alpha = 0.6, scale = 1.0
				if (reg.minTier === stepIndex && stepIndex > 0) { alpha = 0.2 + 0.6 * hp; scale = 0.5 + 0.5 * hp }
				else { alpha = 0.6 + 0.4 * hp }
				...
				if (reg.type === "mega-hub") { ctx.fillRect(x - s, y - s, s * 2, s * 2); ctx.fillRect(x - s * 1.5, y + s * 0.2, s, s); ctx.fillRect(x + s * 0.5, y - s * 1.5, s, s) }
				else if (reg.type === "regional") { ctx.fillRect(x - s, y - s, s * 2, s * 2); ctx.beginPath(); ctx.arc(x, y, s * 0.5, 0, Math.PI * 2); ctx.fillStyle = "#0a1018"; ctx.fill() }
				else { ctx.beginPath(); ctx.arc(x, y, s, 0, Math.PI * 2); ctx.fillStyle = `rgba(${cRGB}, ${alpha})`; ctx.fill() }
				if (hp > 0 && reg.type !== "settlement") { ctx.beginPath(); ctx.fillStyle = `rgba(${cRGB}, ${0.15 * hp})`; ctx.arc(x, y, s * 4, 0, Math.PI * 2); ctx.fill() }
			})
		...
		animationFrameId = requestAnimationFrame(render)
		return () => { cancelAnimationFrame(animationFrameId); window.removeEventListener("resize", resize); observer.disconnect() }
...
const STEPS = [
	{ title: "Current State", description: "Identify isolated regions and infrastructure gaps.", stepIndex: 0 },
	{ title: "Investment Scenario", description: "Plan corridors and bridge critical missing links.", stepIndex: 1 },
	{ title: "Projected Outcome", description: "Achieve fully connected, resilient infrastructure.", stepIndex: 2 },
]
	...
	return (
		...
		>
			...
			<div className="self-start flex flex-col gap-4">
				...
				<SectionBody>
					Simulate infrastructure investments and see the projected impact before it happens.
				</SectionBody>
				<BtnSecondary href="#simulation-tool" id="sim-explore-btn" className="mt-[18px] self-start">
							...
							<React.Fragment key={step.title}>
								<article
									className={cn(
										...
										isHovered && "z-[2] -translate-y-[6px] border-[rgba(146,197,255,0.3)] shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_15px_40px_rgba(0,0,0,0.35)]",
									)}
								...
								{index < STEPS.length - 1 && (
									<span
										...
										className={cn(
											...
											isHovered ? ARROW_COLORS[index] : "text-[#9fb0c8] opacity-50",
											"max-[720px]:justify-self-center max-[720px]:rotate-90",
										)}
										style={isHovered ? { animation: ARROW_KEYFRAME_ANIMATIONS[index] } : undefined}
									>
										<svg
