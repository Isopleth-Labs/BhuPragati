...
function softenGlow(map: MaplibreMap, id: string, width: number, blur: number, opacity: number) {
...
export function softenIntelligenceLayers(map: MaplibreMap) {
  ...
  safe(() => map.setPaintProperty("flood-wetland-points", "circle-stroke-opacity", 0.6));
  ...
  safe(() => map.setPaintProperty("healthcare-access-halos", "circle-blur", 1.2));
  safe(() => map.setPaintProperty("healthcare-access-halos", "circle-opacity", 0.18));
  safe(() => map.setPaintProperty("healthcare-access-points", "circle-blur", 0.3));

  // Agriculture — dependency heat regions
  safe(() => map.setPaintProperty("agriculture-belts-fill", "fill-opacity", 0.13));
