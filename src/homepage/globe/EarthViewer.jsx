...
function orientEarthTowardsIndia(pivot) {
  ...
  const quaternion = new THREE.Quaternion().setFromUnitVectors(target, reference);
...
function buildFillGeometry(polygons, radius) {
  ...
  polygons.forEach((poly) => {
    ...
    const positions = [];
    const pushRing = (ring) => ring.forEach((v) => positions.push(v.x, v.y, v.z));
    const remap = [];
...
function EarthViewer({ className, onReady, onIndiaSelected }) {
  ...
  useEffect(() => {
    ...
    console.warn('[country-overlays] temporarily hidden (lines/fills) to debug globe lighting/texture');
    ...
    const formatQuat = (q) => [q.x, q.y, q.z, q.w].map((v) => Number(v.toFixed(6)));
    const formatEulerDeg = (e) => [e.x, e.y, e.z].map((rad) => Number(THREE.MathUtils.radToDeg(rad).toFixed(3)));
    ...
    const setHoveredCountry = (entry) => {
      ...
      if (hoveredCountry) {
        hoveredCountry.line.material = hoveredCountry === selectedCountry ? selectedBorderMaterial : borderMaterial;
        hoveredCountry.fill.material = hoveredCountry === selectedCountry ? selectedFillMaterial : fillMaterial;
      }
      hoveredCountry = entry;
      if (hoveredCountry) {
        hoveredCountry.line.material = hoverBorderMaterial;
        hoveredCountry.fill.material = hoveredCountry === selectedCountry ? selectedFillMaterial : hoverFillMaterial;
      }
    };
    ...
    const loadCountries = async () => {
      try {
        ...
        features.forEach((feature, featureIndex) => {
          ...
          const borderGeometry = buildBorderGeometry(polygons, EARTH_RADIUS * BORDER_OFFSET);
          const fillGeometry = buildFillGeometry(polygons, EARTH_RADIUS * BORDER_OFFSET);
          const isIndia = name === 'India';
          const lineMaterial = isIndia ? new THREE.LineBasicMaterial({ color: 0xff3b3b, transparent: true, opacity: 0.9, depthTest: true, depthWrite: true }) : borderMaterial.clone();
          ...
          const centroidVec = computeCentroidVector(polygons, EARTH_RADIUS * LABEL_OFFSET);
          const centroidLat = THREE.MathUtils.radToDeg(Math.asin(centroidVec.clone().normalize().y));
          const centroidLon = THREE.MathUtils.radToDeg(Math.atan2(centroidVec.z, centroidVec.x));
    ...
    const rotateState = { isDragging: false, lastX: 0, lastY: 0, inertiaX: 0, inertiaY: 0 };
    const damping = 0.92;
    const minRadius = EARTH_RADIUS * 3.0;
    const maxRadius = EARTH_RADIUS * 5.2;
    spherical.set(camera.position.length(), Math.acos(camera.position.y / camera.position.length()), Math.atan2(camera.position.x, camera.position.z));
    ...
    const updateSelectedLabel = () => {
      ...
      const worldPos = centroidDir.clone().multiplyScalar(EARTH_RADIUS * LABEL_OFFSET);
    ...
    const renderLoop = () => {
      ...
      if (!rotateState.isDragging) {
        ...
        if (Math.abs(rotateState.inertiaX) > 0.00001 || Math.abs(rotateState.inertiaY) > 0.00001) {
          applyCameraPosition();
        }
      }
