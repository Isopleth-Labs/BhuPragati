...
import { INDIA_INITIAL_VIEW_STATE, INDIA_MAX_BOUNDS, INDIA_ZOOM_LIMITS } from "@/shared/lib/config/mapConfig";
...
export default function StateMapShell({ onBiharDrillDown }: StateMapShellProps) {
  ...
  const mapSlot = (
    <Suspense fallback={<div className="state-map__fallback" />}>
      <MapEngine regionId="india" initialViewState={INDIA_INITIAL_VIEW_STATE} maxBounds={INDIA_MAX_BOUNDS} zoomLimits={INDIA_ZOOM_LIMITS}>
        <LabelTuningController />
        <StateBoundaryLayer onReady={() => setStateLayerReady(true)} />
        <ChoroplethUpdateController activeIndicator={activeIndicator} ready={stateLayerReady} />
        <StateSelectionController ready={stateLayerReady} onStateClick={handleStateClick} />
      </MapEngine>
    </Suspense>
  );
