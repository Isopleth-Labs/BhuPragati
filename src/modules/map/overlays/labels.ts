...
function tuneCityLayer(map: MaplibreMap, id: string) {
	safeSet(() =>
	...
	safeSet(() =>
		map.setPaintProperty(id, "text-color", "rgba(255, 255, 255, 0.97)"),
	);
...
function _tuneTownLayer(map: MaplibreMap, id: string) {
	safeSet(() =>
		map.setLayoutProperty(id, "text-size", [
			"interpolate",
			["linear"],
	...
	safeSet(() =>
		map.setPaintProperty(id, "text-color", "rgba(230, 238, 250, 0.84)"),
	);
	safeSet(() => map.setPaintProperty(id, "text-halo-color", HALO_DARK));
...
function _tuneVillageLayer(map: MaplibreMap, id: string) {
	safeSet(() =>
		map.setLayoutProperty(id, "text-size", [
			"interpolate",
			["linear"],
	...
	safeSet(() =>
		map.setPaintProperty(id, "text-color", "rgba(190, 205, 225, 0.64)"),
	);
...
function tuneRoadLayer(map: MaplibreMap, id: string) {
	safeSet(() =>
		map.setPaintProperty(id, "text-color", "rgba(170, 188, 210, 0.5)"),
	);
	safeSet(() => map.setPaintProperty(id, "text-halo-color", HALO_SOFT));
...
function tuneWaterLabel(map: MaplibreMap, id: string) {
	safeSet(() =>
		map.setPaintProperty(id, "text-color", "rgba(150, 205, 240, 0.78)"),
	);
	safeSet(() =>
		map.setPaintProperty(id, "text-halo-color", "rgba(2, 8, 16, 0.88)"),
	);
	safeSet(() => map.setPaintProperty(id, "text-halo-width", 1));
	safeSet(() => map.setLayoutProperty(id, "text-letter-spacing", 0.05));
}
...
function tuneWaterFill(map: MaplibreMap, id: string) {
...
function tuneWaterLine(map: MaplibreMap, id: string) {
	safeSet(() =>
		map.setPaintProperty(id, "line-color", "rgba(180, 220, 244, 0.92)"),
	);
	safeSet(() =>
...
export function tuneMapLabels(map: MaplibreMap) {
	...
	for (const layer of style.layers) {
		...
		if (type === "symbol") {
			...
			if (/place_country|place_state|place_continent/i.test(id)) {
				safeSet(() =>
					map.setPaintProperty(id, "text-color", "rgba(210, 222, 240, 0.7)"),
				);
				safeSet(() => map.setPaintProperty(id, "text-halo-color", HALO_SOFT));
				safeSet(() => map.setPaintProperty(id, "text-halo-width", 1.1));
				safeSet(() => map.setLayoutProperty(id, "text-letter-spacing", 0.08));
			} else if (
				/place_town|place_village|place_hamlet|place_suburb|place_neighbour|place_other/i.test(
					id,
				)
			) {
			...
			type === "line" &&
			/water|river|stream|waterway|canal/i.test(id)
		) {
...
export function applyTacticalSky(map: MaplibreMap) {
	try {
		map.setSky?.({
			"sky-color": "#06121f",
