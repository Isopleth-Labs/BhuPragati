...
import { addLayer, addSource, getFirstSymbolLayerId } from "@/shared/lib/utils/utils";
...
function applyChoroplethScores(map: MaplibreMap, indicator: IndicatorKey): void {
...
export function StateBoundaryLayer({ onReady }: StateBoundaryLayerProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled || !source || source.type !== "geojson" || !source.data) return;
      ...
      const labelFeatures: GeoJSON.Feature[] = data.features.map((f) => {
        ...
        return {
          ...
          properties: { id: p.id, name_en: p.name_en, label_tier: p.label_tier },
        };
      });
        ...
        layout: {
          ...
          "text-size": [
            ...
            [
              ...
              "kerala", "jammu-and-kashmir", "ladakh", "uttarakhand",
              "himachal-pradesh", "assam", "arunachal-pradesh",
            ], 12,
            [
              "delhi", "chandigarh", "goa", "sikkim", "puducherry",
              "lakshadweep", "andaman-and-nicobar-islands", "meghalaya",
            ], 10,
            9,
          ],
          "text-offset": [
